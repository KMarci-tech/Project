...
public class Player extends GameObject{
	...
	public Player(float x, float y,Handler handler,Camera cam, HUD hud, ObjectId id) {
		super(x, y, id);
		this.hud = hud;
		//hud.setHealth(100);
		hud.health = 100;
	...
	private void Collision(LinkedList<GameObject> object) {
		...
		for(int i = 0; i < handler.object.size(); i++) {
			...
			if(tempObject.getId() == ObjectId.Sign) {
				...
				if(getBounds().intersects(tempObject.getBounds()) && hud.coins == 5) {
					handler.clearLevel();
					cam.setX(0);
	...
	public void dead() {
		...
		if(hud.lifes == 1) {
			...
			hud.lifes--;
			handler.addObject(new Player(startX,startY,handler,cam,hud,ObjectId.Player));
		}
